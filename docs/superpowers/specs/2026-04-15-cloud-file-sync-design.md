...
```
1. 内存中保留该目录及其文件的meta信息
2. 同步时遍历内存中的meta记录：
   a. 检查本地文件是否实际存在
   b. 若不存在，比较"本地目录删除时间"与"文件最后修改时间"
      - 删除时间 > 文件最后修改时间：确认为已确认的删除
      - 否则：可能是重命名或移动，暂不处理
3. 若为已确认删除：
   a. 检查云端对应文件是否存在
   b. 若存在且hash匹配：删除云端文件，保留meta文件
   c. 若不存在：跳过
4. 目录结构在云端永不删除
```
...
8. 若云端文件已删除，且本地剩余meta文件也被手动删除，将无法同步删除操作；另一客户端若有该文件，将重新上传
